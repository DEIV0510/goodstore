...
for(const [f,cfg] of Object.entries(IMGS)){
  ...
  // uniform fit for rows
  const fitLine=(prof,n,g0,g1,span)=>{
    let best=null;
    for(let s=g0-span;s<=g0+span;s++){
      for(let e=g1-span;e<=g1+span;e++){
        if(e<=s+n*20) continue;
        const p=(e-s)/n; let sc=0;
        ...
        if(!best||sc>best.sc) best={sc,s,e,p};
      }
    }
    return best;
  };
  ...
  ys.slice(0,-1).forEach((y0,ri)=>{
    ...
    const cf=fitLine(vs,n,x0,x1,18);
    const xs=Array.from({length:n+1},(_,i)=>Math.round(cf.s+cf.p*i));
    rows.push({y0,y1,xs,pitch:+cf.p.toFixed(1)});
  });
  out[f]={off:cfg.off,size:cfg.size,rows};
  console.log(f,'content',[cx0,cy0,cx1,cy1].join(','),'rowPitch',rf.p.toFixed(1));
